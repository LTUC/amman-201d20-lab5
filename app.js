...
function sumAndMultiply(a, b, c) {
  //eslint-disable-line
let sum1=sum(a,b)[0];
let sumTotal = sum(sum1,c)[0];
let multiply1= multiply(a,b)[0];
let multiplyTotal=multiply(multiply1,c)[0];
...
function sumArray(sumArr) {
  ...
  let sumFinal=0;
  for(let i=0; i<3;i++){
   sumFinal=sum(sumFinal,sumArr[i])[0];
  }
...
function multiplyArray(multArr) {
  ...
  for(let i=0;i<multArr.length;i++){
    multFinal=multiply(multFinal,multArr[i])[0];
  }
  return[multFinal,`The numbers ${multArr} have a product of ${multFinal}.`]
}
